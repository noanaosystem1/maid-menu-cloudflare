...
### 4.2 ルーム新規作成（疎結合 API）
外部システム等から新しくルーム（座席・テーブル）を単体で登録し、自動生成されたルームの UUID を返却します。
この UUID を使うことで、後から `POST /api/guests` 経由でゲストを紐づけて個別登録することができます。
...
### 5.2 ゲスト個別登録（疎結合 API）
外部システム等から特定のルームに対して個別にユーザーを追加し、専用アクセスURL（ゲストURL）を自動で発行します。
これにより、「部屋の作成」と「人の登録（あとからメンバーを追加）」を完全に疎結合かつ1回分のワーカー消費（1 invocation）で実行できます。
...
- **リクエスト Body (JSON)**:
  ...
  ```
  - `name` (string, **必須**): 追加するゲストの名前。
  - `roomId` (string, **必須**): ゲストが所属する部屋のUUID。
  - `id` (string, 任意): ゲストUUID。未指定時はシステム側で自動採番されます。
  - `sessionToken` (string, 任意): 一意のトークン文字列。未指定時はセキュアな UUIDv4 が自動生成されます。

- **成功レスポンス 201 (Created)**:
  ```json
  {
    "id": "2da2f170-c081-4bc1-aa8f-12cc9ef80cc5",
    "name": "ひなた",
    "roomId": "96ca036a-222c-4396-b0bd-2050fd8c1987",
    "sessionToken": "b478dcb1-44bf-42f0-bcde-ff81432f81cc",
    "isActive": true,
    "isOnline": false,
    "lastSeen": null,
    "created_date": "2026-07-12T05:22:15.341Z",
    "guestUrl": "http://localhost:8787/guest?token=b478dcb1-44bf-42f0-bcde-ff81432f81cc"
  }
  ```
  - **レスポンス特典:** 自動生成されたセキュアトークンに基いて、そのままブラウザで開けるゲスト専用URL `guestUrl` が、作成されたゲスト情報にアタッチされて返却されます。
